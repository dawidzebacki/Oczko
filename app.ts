...
const changeHiddenCard = (url: string) => {
  const image = <HTMLInputElement>document.getElementById("hidden_card");
  image.src = url;
};
...
const showMenuMultiplayer = () => {
  ...
  setInputFilter(document.getElementById("number_of_players"), function (
    value
  ) {
...
function addImage(url: string, whichCards: string, hiddenCard: boolean) {
...
const disableHitAndStand = () => {
  const hitBtn = <HTMLInputElement>document.getElementById("hit_button");
  const standBtn = <HTMLInputElement>document.getElementById("stand_button");
...
const checkPoints = (cardValue: string, howManyCards: number) => {
...
const checkWinner = () => {
  ...
  let sortedWinners = results
    ...
    .sort((a, b) => b[1] - a[1]);
  sortedWinners = sortedWinners.filter(
    (element) => element[1] === sortedWinners[0][1]
  );
